...
/**
 * MVI contract for the post-workout SUCCESS screen (per-screen, not a shared
 * generic model). Public rather than internal because the native iOS/Android
 * hosts read [ViewModel.viewState] and call [ViewModel.dispatch] across the
 * SKIE bridge. The screen has no one-shot outputs, so there is no `viewEffect`.
 */
object WorkoutSuccessContract {
    ...
    data class ViewState(
        ...
        /** One-shot success haptic on first appearance — consume via [ViewAction.SuccessHapticPlayed]. */
        val playSuccessHaptic: Boolean = false,
    )

    /** Every interaction on the success screen — the single input to [ViewModel.dispatch]. */
    sealed interface ViewAction {
        /** The composable has played the one-shot success haptic; clear the flag. */
        data object SuccessHapticPlayed : ViewAction
    }
}
