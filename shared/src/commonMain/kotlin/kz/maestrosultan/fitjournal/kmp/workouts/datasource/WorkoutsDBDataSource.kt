package kz.maestrosultan.fitjournal.kmp.workouts.datasource
...
import kz.maestrosultan.fitjournal.kmp.WorkoutExercisesQueries
import kz.maestrosultan.fitjournal.kmp.WorkoutRecordsQueries
import kz.maestrosultan.fitjournal.kmp.WorkoutSetsQueries
import kz.maestrosultan.fitjournal.kmp.time.toStoredString
import kz.maestrosultan.fitjournal.kmp.workouts.entity.DBWorkoutExerciseObject
...
import kz.maestrosultan.fitjournal.kmp.workouts.entity.DBWorkoutRecordRow
import kz.maestrosultan.fitjournal.kmp.workouts.entity.DBWorkoutSetObject
import kz.maestrosultan.fitjournal.kmp.workouts.entity.map

/**
 * KMP wrapper around the workout SQLDelight tables (workoutRecords,
 * workoutExercises, workoutSets). Sole source of truth for workout state
 * on-device. Reads filter tombstones (`deletedAt IS NULL`) at the SQL level.
 *
 * All multi-table writes (the migrator's per-record import, the repo's
 * createWorkout) happen inside a single SQLDelight transaction so partial
 * failures roll back cleanly.
 *
 * For sync (Round 5/6): only `workoutRecords` carries `pendingUpload` —
 * children are part of the record's JSON when pushed to AWS, so any change
 * to a child marks the parent record `pendingUpload=1` (callers do this).
 */
class WorkoutsDBDataSource(
    ...
    companion object {
        // Date-window sentinels for callers that want "everything in the
        // diary" without making the query signature optional. Matches the
        // ISO yyyy-MM-dd lex-compare used throughout.
    ...
    /**
     * Most recent set with [exerciseUuid] in this diary whose parent
     * record date is strictly before [beforeDateString] (`yyyy-MM-dd`).
     * Drives the `previousWeight` / `previousDistance` / `previousDifficultyType`
     * hints rendered next to a workout-set cell. Null if the user has
     * never logged this exercise before this date.
     */
    ...
    suspend fun replaceWorkoutRecord(record: DBWorkoutRecord) = withContext(Dispatchers.IO) {
        recordsDao.transaction {
            ...
            // Cascade: workoutSets has ON DELETE CASCADE on its FK to
            // workoutExercises, so wiping exercises wipes their sets too.
    ...
    suspend fun deleteAllForUser(userId: String) = withContext(Dispatchers.IO) {
        recordsDao.transaction {
            // Children cascade via FK ON DELETE CASCADE when records go.
    ...
    /** Insert the parent row + every child exercise + its sets. Caller wraps in a transaction. */
    ...
    /** Insert every child exercise + its sets. Caller wraps in a transaction. */
