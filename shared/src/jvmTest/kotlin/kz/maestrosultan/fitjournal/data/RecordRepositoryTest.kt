...
class RecordRepositoryTest {
    ...
    @Test
    fun copyWorkoutTo_anExistingPage_appendsAfterItsRows(): Unit = runBlocking {
        // Append after what is there, never a second page. Positions continue.
        val exId = seedCatalogExercise()
        val target = LocalDate(2026, 5, 1)
        repo.addExercisesToDate(userId, journalId, date, 2, listOf(exId))
        repo.addExercisesToDate(userId, journalId, target, 1, listOf(exId))

        assertTrue(repo.copyWorkoutTo(userId, journalId, date, 2, target, 1))

        val onTarget = repo.getRecordsByDate(userId, journalId, target)
        assertEquals(setOf(1), onTarget.map { it.workoutNumber }.toSet(), "one page only")
        assertEquals(2, onTarget.size, "appended, not merged into the existing record")
        assertEquals(listOf(0, 1), onTarget.map { it.position }.sorted(), "positions continue")
    }

    @Test
    fun copyWorkoutTo_aFreshWorkoutNumber_landsABlankTemplate(): Unit = runBlocking {
        val exId = seedCatalogExercise()
        val target = LocalDate(2026, 5, 2)
        repo.addExercisesToDate(userId, journalId, date, 1, listOf(exId))
        val weId = repo.getRecordsByDate(userId, journalId, date).single().exercises.single().id
        repo.addSet(userId, journalId, weId, 100.0, 5, null, null)
        repo.addSet(userId, journalId, weId, 110.0, 3, null, null)

        assertTrue(repo.copyWorkoutTo(userId, journalId, date, 1, target, 4))

        val copied = repo.getRecordsByDate(userId, journalId, target).single()
        assertEquals(4, copied.workoutNumber, "lands on the requested fresh page")
        val sets = copied.exercises.single().sets
        assertEquals(2, sets.size, "set count is preserved")
        assertTrue(sets.all { it.weight == null && it.reps == null }, "values are cleared into a template")
    }

    @Test
    fun copyWorkoutTo_copiesEveryExerciseWithItsSetCount_noneDropped(): Unit = runBlocking {
        val exA = seedCatalogExercise()
        val exB = seedCatalogExercise()
        val target = LocalDate(2026, 5, 3)
        ...
        repo.addSet(userId, journalId, records[0].exercises.single().id, 50.0, 10, null, null)
        repo.addSet(userId, journalId, records[1].exercises.single().id, 60.0, 8, null, null)
        repo.addSet(userId, journalId, records[1].exercises.single().id, 65.0, 6, null, null)

        assertTrue(repo.copyWorkoutTo(userId, journalId, date, 1, target, 1))

        val sourceCounts = records.map { it.exercises.single().sets.size }.sorted()
        ...
        assertEquals(
            sourceCounts,
            copied.map { it.exercises.single().sets.size }.sorted(),
            "per-exercise set counts match the source",
        )
    }

    @Test
    fun copyWorkoutTo_emptySource_returnsFalse_andWritesNothing(): Unit = runBlocking {
        val target = LocalDate(2026, 5, 4)

        val result = repo.copyWorkoutTo(userId, journalId, date, 99, target, 1)

        assertEquals(false, result, "no source records -> nothing written")
        assertTrue(repo.getRecordsByDate(userId, journalId, target).isEmpty())
    }
